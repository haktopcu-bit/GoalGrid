...
# Remove every remaining Sofascore label/reference from the UI and logic.
s=s.replace('      <span class="source">Sofascore • Takım geçmişi fallback</span>\n','')

# Remove stale fallback block that still called the deleted loadSofaHistory().
...
# Safety: if a different stale block survived, strip any direct loadSofaHistory call section.
