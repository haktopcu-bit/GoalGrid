...
# Remove failing Sofascore fallback from active flow; 403s are not useful to the user.
start=s.find('async function loadSofaHistory(teamName){')
end=s.find('\n\nfunction mergeMatches(...groups){',start)
if start!=-1 and end!=-1:
    s=s[:start]+s[end+2:]

# Robust team-name matching: provider IDs differ, names should match by aliases/tokens.
...
new=r'''function sameTeamName(a,b){
  ...
  // Strong first meaningful token fallback (Celta Vigo vs RC Celta de Vigo, Getafe vs Getafe CF, etc.)
  return tx[0]===ty[0] && tx[0].length>=5;
}'''
s=s[:start]+new+s[end:]

# Re-enable API-Football history using ONLY seasons allowed by the Free plan (2024 then 2023).
...
# Fresh caches so old bad provider matching does not persist.
s=s.replace('goalgrid_fd_v181_','goalgrid_fd_v110_')
s=s.replace('goalgrid_of_v181_','goalgrid_of_v110_')

# Replace Sofascore fallback block in buildFixtureHistory with accessible API-Football Free-season fallback.
...
# Update the API fallback comment and keep logic active.
...
# Remove Sofascore source classifier if present.
