...
if(typeof oldWanted==='function'){
  window.wantedFixture=function(f){
    ...
    if(is2Bundesliga(f)) return new Date(f?.fixture?.date||'').getTime()>Date.now();
    return oldWanted(f);
  };
}
...
async function directApi(team,date){
  ...
  try{
    ...
    if(c&&Date.now()-c.time<3*60*60*1000&&Array.isArray(c.rows))return c.rows.filter(x=>String(x.date||'')<date);
  }catch{}
  try{
...
async function directSofa(team,date){
  ...
  try{
    const c=JSON.parse(localStorage.getItem(key)||'null');
    if(c&&Date.now()-c.time<2*60*60*1000)payload=c.payload;
  }catch{}
  if(!payload){
    try{
  ...
  return (payload.matches||[]).filter(m=>finished(m)&&String(m.date||'')<date).map(m=>{
    if(m.subjectSide==='home')return {...m,team1Id:Number(team.id),team2Id:m.sofaAwayId??null,source:'Sofascore'};
    if(m.subjectSide==='away')return {...m,team1Id:m.sofaHomeId??null,team2Id:Number(team.id),source:'Sofascore'};
    return m;
  });
}
...
async function domesticBuild(fixture){
  ...
  if(!h||!a)return null;
  const cfg=competitionConfig(fixture);
  const groups=[],sources=[],errors=[];
  ...
  // Sofascore is deliberately queried even when the first sources returned data.
  // It provides a second independent history and fixes clubs absent from OpenFootball.
...
window.buildFixtureHistory=async function(fixture){
  ...
  if(clean&&clean.matches.length)return clean;
  try{return await oldBuild(fixture);}catch{return clean||{matches:[],sources:[],errors:['Geçmiş veri kaynakları yanıt vermedi.']};}
};

// The old UI said VERİ YETERSİZ whenever fewer than two matches were available.
// Keep the calculation honest, but make the failure reason diagnostic instead of a generic blocker.
const oldAnalyse=window.analyseFixture;
if(typeof oldAnalyse==='function'){
  window.analyseFixture=async function(fixture,history,sources){return oldAnalyse(fixture,history,sources);};
}
